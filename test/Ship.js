...
describe('Ship', () => {
    ...
    describe('#constructor()', () => {
        ...
        it('works with valid coordinates', () => {
            let bow = { x: 0, y: 0 };
            let stern = { x: 0, y: 1 };
            let s = new Ship({ bow: bow, stern: stern });
        ...
        it('fails when a ship is too wide', () => {
            let bow = { x: 0, y: 0 };
            let stern = { x: 1, y: 1 };
            assert.throws(() => { new Ship({ bow: bow, stern: stern })});
        });
    ...
    describe('#collides()', () => {
        it('does not collide ships which do not overlap', () => {
            let s1 = new Ship({ bow: { x: 0, y: 0 }, stern: { x: 0, y: 1 } });
            let s2 = new Ship({ bow: { x: 1, y: 0 }, stern: { x: 1, y: 1 } });
            assert(!s1.collides(s2));
            assert(!s2.collides(s1));
        });
        it('collides ships which do overlap', () => {
            let s1 = new Ship({ bow: { x: 0, y: 0 }, stern: { x: 0, y: 1 } });
            let s2 = new Ship({ bow: { x: 0, y: 1 }, stern: { x: 0, y: 2 } });
            assert(s1.collides(s2));
            assert(s2.collides(s1));
        });
    });
    ...
    describe('#hit()', () => {
        ...
        it('does not hit with a bad shot', () => {
            let s = new Ship({ bow: { x: 0, y: 0 }, stern: { x: 0, y: 1 } });
            let shot = new Coordinate({ x: 1, y: 1});
            assert(!s.hit(shot));
        });
    ...
    describe('#sunk', () => {
        ...
        it('does sink a ship with all hits', () => {
            let s = new Ship({ bow: { x: 0, y: 0 }, stern: { x: 0, y: 1 } });
            let shot1 = new Coordinate({ x: 0, y: 0 });
            assert(s.hit(shot1));
            let shot2 = new Coordinate({ x: 0, y: 1 });
            assert(s.hit(shot2));
            assert(s.sunk);
        });
